...
def createFPTree(frozenDataSet, minSupport):
    ...
    for transactions, count in frozenDataSet.items(): #再次scanfrozenDataSet中的所有items
        ...
        if len(frequentItemsInRecord) > 0:
            ...
            orderedFrequentItems = [v[0] for v in sorted(frequentItemsInRecord.items(), key=lambda v: v[1], reverse=True)] # sort by v[1] (support)降序
...
def updateFPTree(fptree, orderedFrequentItems, frequenctNodeTable, count):
    ...
    if firstOrderItem in fptree.children: # 如果firstOrderItem是fptree 節點的子節點
        ...
        fptree.children[firstOrderItem] = TreeNode(firstOrderItem, count, fptree) #否則，需要新創建一個TreeNode 節點，然後將其賦給fptree 節點的子節點
        # update frequenctNodeTable
        if frequenctNodeTable[firstOrderItem][1] == None: # 如果frequenctNodeTable裡記錄的firstOrderItem Node為None
            frequenctNodeTable[firstOrderItem][1] = fptree.children[firstOrderItem] # 則更新frequenctNodeTable裡的firstOrderItem Node為自己本身
        else:
            updateFrequenctNodeTable(frequenctNodeTable[firstOrderItem][1], fptree.children[firstOrderItem])  #更新 firstOrderItem 下一個相似元素指標
...
def mining_FPTree(frequenctNodeTable, suffix, frequentPatterns, minSupport):
    ...
    frequenctItems = [v[0] for v in sorted(frequenctNodeTable.items(), key=lambda v: v[1][0])] # 將frequenctNodeTable以升序排序，從low frequent開始mining
    ...
    for frequenctItem in frequenctItems:
        ...
        if (suffixPath != {}):
            ...
            if conditionalFNodeTable != None: # 假如 conditional frequenctNode不為空
                mining_FPTree(conditionalFNodeTable, newSuffix, frequentPatterns, minSupport) #繼續挖掘剩餘的conditionalFPtree
...
def getSuffixPath(frequenctNodeTable, frequenctItem):
    ...
    if ((suffixs != [])):
        suffixPath[frozenset(suffixs)] = beginNode.count  # 建立一條由suffix Node起始但不包含suffix Node的完整suffixPath，數量為beginNode的出現次數
...
if __name__ == '__main__':
    ...
    # python FP-Growth.py BreadBasket_DMS.csv 3 0.6
